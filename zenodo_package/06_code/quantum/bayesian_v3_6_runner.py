...
def build_model(df: pd.DataFrame, beta_xi_mean: float, beta_xi_sd: float, xi_baseline_nm: float, baseline_naacr: float):
    ...
    with pm.Model() as model:
        ...
        # Latent xi per phase (nm) with weakly informative priors around plausible ranges
        xi_nm_ctrl = pm.Normal('xi_nm_control', mu=0.66, sigma=0.10)
        xi_nm_acute = pm.Normal('xi_nm_acute', mu=0.61, sigma=0.10)
        xi_nm_chron = pm.Normal('xi_nm_chronic', mu=0.80, sigma=0.10)
        xi_nm = pm.math.stack([xi_nm_ctrl, xi_nm_acute, xi_nm_chron])

        # Protection factor per phase
        Pi_xi_phase = (xi_nm / xi_baseline_nm) ** (-beta_xi)
