...
\lhead{Noise Decorrelation in HIV}
\rfoot{Page \thepage}
...
\large Noise Decorrelation as a Hypothetical Mechanism for\\Phase-Specific Neurometabolic Outcomes in HIV Infection:\\A Computational Framework}

\author{A.C. Demidont, DO\\
Independent Researcher, Nyx Dynamics\\
268 Post Rd, Suite 200, Fairfield, CT 06428\\
...
\begin{itemize}
    ...
    \item Target acceptance rate: 0.92
...
\begin{table}[h]
\centering
\caption{Prior sensitivity analysis results}
\begin{tabular}{lcccc}
\toprule
\textbf{Prior Type} & $\xi_\text{acute}$ (nm) & SD & $P(\xi_a < \xi_c)$ & Interpretation \\
\midrule
Weakly informative (default) & 0.425 & 0.065 & 0.9997 & Decisive \\
Flat/Uniform & 0.431 & 0.072 & 0.9989 & Decisive \\
Literature-based (tight) & 0.419 & 0.058 & 0.9999 & Decisive \\
Skeptical (narrow) & 0.440 & 0.051 & 0.9982 & Decisive \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Conclusion}: Results are robust to prior specification. The range of $\xi_\text{acute}$ estimates across priors is $<0.025$ nm ($<6\%$ variation), and all scenarios yield decisive evidence for the protective effect ($P > 0.998$).
...
Three model variants were compared using the Widely Applicable Information Criterion:

\begin{table}[h]
\centering
\caption{WAIC model comparison}
\begin{tabular}{lccccc}
\toprule
\textbf{Model} & \textbf{WAIC} & \textbf{SE} & $\Delta$\textbf{WAIC} & \textbf{Weight} & \textbf{$p_\text{WAIC}$} \\
\midrule
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figures/legacy/SuppFig_prior_sensitivity.png}
\caption{\textbf{Prior sensitivity analysis.} Posterior distributions for $\xi_\text{acute}$ under four alternative prior specifications. All priors yield consistent estimates with overlapping 95\% HDIs, demonstrating robustness to prior choice.}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figures/legacy/SuppFig_power_analysis.png}
\caption{\textbf{Statistical power analysis for future studies.} Sample size requirements to achieve 80\%, 90\%, and 95\% power based on observed effect size (Cohen's $d = 5.63$). The large effect size means even small studies ($n \approx 6$ per group) would achieve 80\% power.}
\label{fig:power}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figures/legacy/SuppFig_loso_analysis.png}
\caption{\textbf{Leave-one-study-out sensitivity analysis.} Forest plot showing $\xi_\text{acute}$ estimates when excluding each study. The consistency across subsets demonstrates that no single cohort drives the overall conclusions.}
\label{fig:loso}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figures/legacy/SuppFig_subgroup_analysis.png}
\caption{\textbf{Subgroup analysis by brain region.} NAA/Cr ratios across infection phases for different anatomical regions. The acute $\geq$ control pattern is consistent across basal ganglia, frontal white matter, and anterior cingulate regions.}
\label{fig:subgroup}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figures/legacy/Version_Comparison_v28_vs_v36.png}
\caption{\textbf{Model version comparison.} Comparison of posterior estimates between v2.8 (individual-level) and v3.6 (hierarchical) model implementations. Both approaches yield qualitatively similar conclusions regarding phase-specific $\xi$ differences.}
...
All source code is available at: \url{https://github.com/Nyx-Dynamics/noise_decorrelation_hiv}
...
\begin{enumerate}
    ...
    \item Run the main Bayesian analysis: \\
    \texttt{python -m quantum.bayesian\_v3\_6\_runner --ratio 3\_1\_1}

    \item Generate statistical report: \\
    \texttt{python scripts/statistical\_improvements.py}
\end{enumerate}

All random seeds are fixed for reproducibility. Expected runtime on a modern workstation (8+ cores): approximately 15--30 minutes for MCMC sampling.
