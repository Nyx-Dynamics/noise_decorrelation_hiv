...
\subsection{S1: Expanded Dataset Compilation}

\subsubsection{S1.1 Individual Patient Data Extraction}

We extracted individual patient data from published supplementary materials using systematic protocols:

\textbf{Valcour et al. 2015 (n=44 acute HIV):}
\begin{itemize}
\item Source: Supplementary Table 3 from Thai RV254 cohort
\item Time point: Median 20 days post-infection (range: 14--30)
\item Regions: Basal ganglia (BG), frontal gray matter (FGM), frontal white matter (FWM), posterior gray matter (PGM)
\item Metabolites: NAA, Cho, Cr absolute concentrations (mM)
\item Conversion: Ratios calculated as NAA/Cr, Cho/Cr
\end{itemize}

\textbf{Data extraction methodology:}
\begin{enumerate}
\item Digital extraction from PDF supplementary materials
\item Quality control: Cross-validation with reported group means
\item Missing data: Handled via listwise deletion (< 5\% missing)
\item Outlier detection: Winsorization at 3 standard deviations
\end{enumerate}

\subsubsection{S1.2 Group-Level Data Harmonization}

For studies reporting only group means and standard errors, we employed Monte Carlo expansion:
\begin{equation}
X_i \sim \text{Normal}(\mu_{\text{group}}, \text{SE}_{\text{group}} \times \sqrt{n})
\end{equation}
where $X_i$ represents synthetic individual observations, $\mu_{\text{group}}$ is the reported mean, and SE is the standard error.

\textbf{Young et al. 2014 (n=53 acute, n=18 chronic, n=19 control):}
\begin{itemize}
\item Regions: Anterior cingulate (AC), FWM, PGM
\item Reported as: NAA/Cr ratios with standard errors
\item Expansion: 1000 Monte Carlo samples per group
\end{itemize}

\textbf{Sailasuta et al. 2016 (group means):}
\begin{itemize}
\item Region: Basal ganglia
\item Phases: Acute (n=35), Chronic (n=26)
\item Time points: Baseline, 24 weeks, 48 weeks
\end{itemize}
